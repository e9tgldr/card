...
begin
  ...
  if not found then
    insert into public.active_sessions (user_id, session_id, last_seen, device_label)
      values (p_user_id, p_session_id, v_now, p_device_label);
    return query select true, null::uuid, p_session_id, v_now, p_device_label;
    return;
  end if;
  ...
  if v_existing.ls < now() - make_interval(secs => p_stale_seconds) or p_force then
    ...
    return query
      select true,
             ...
             p_session_id, v_now, p_device_label;
    return;
  end if;
